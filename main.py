...
all_movies = movies_data[["original_title","poster_link",
                          "release_date","runtime","weighted_rating"]]
...
def recommended_movies():
    try:
        global liked_movies
        col_names=['original_title', 'poster_link', 'release_date',
                    'runtime', 'weighted_rating']
        all_recommended = pd.DataFrame()
        for liked_movie in liked_movies:
            output = get_recommendations(liked_movie["original_title"])
            
            # Concatenar os DataFrames
            all_recommended = pd.concat([all_recommended, output], ignore_index=True)
...
print("Current working directory:", os.getcwd())  # Exibe o diretório de trabalho atual
print("CSV File exists:", os.path.exists('final.csv'))  # Verifica se o arquivo CSV existe
